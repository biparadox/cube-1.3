...
static CHANNEL * websocket_msgport;
...
int websocket_send_func(void * sub_proc);
...
int websocket_msgport_init(void * sub_proc,void * para)
{
...
int websocket_msgport_start(void * sub_proc,void * para)
{
...
//	printf("%s\n",buffer);
	...
	while(level<len)
	{
		...
		if(substr!=NULL)
		{
			...
			print_cubeaudit(" websocket_msgport: get websocket protocol %s",sec_protocol);	
		}
	}
	...
	if(len<0)
	{
		print_cubeerr("write data to websocket_msgport failed, write len %d",strlen(head));
	}
	return 0;
}
