...
static void sm3_process( sm3_context_ext *ctx, unsigned char data[64] )
{
...
void sm3_finish_ext( sm3_context_ext *ctx, unsigned char output[32] )
{
    ...
    sm3_update_ext( ctx, (unsigned char *) sm3_padding, padn );
    sm3_update_ext( ctx, msglen, 8 );
           ...
           unsigned char output[32] )
{
    ...
    memset( &ctx, 0, sizeof( sm3_context_ext ) );
}
...
int calculate_context_sm3(BYTE* context, int context_size, unsigned char * SM3_hash)
{
   
    int result;
    sm3_context_ext index;
    sm3_starts_ext(&index);
    sm3_update_ext( &index,context,context_size);
    sm3_finish_ext( &index,SM3_hash);

    memset( &index, 0, sizeof( sm3_context_ext ) );
    return 0;
}

int calculate_sm3( char *path, BYTE * output )
{
    FILE *f;
    size_t n;
    sm3_context_ext ctx;
...
void sm3_hmac_finish_ext( sm3_context_ext *ctx, unsigned char output[32] )
{
    ...
    sm3_finish_ext( ctx, tmpbuf );
    sm3_starts_ext( ctx );
    sm3_update_ext( ctx, ctx->opad, 64 );
    sm3_update_ext( ctx, tmpbuf, hlen );
    sm3_finish_ext( ctx, output );
                ...
                unsigned char output[32] )
{
    ...
    sm3_hmac_starts_ext( &ctx, key, keylen);
    sm3_hmac_update_ext( &ctx, input, ilen );
    sm3_hmac_finish_ext( &ctx, output );
